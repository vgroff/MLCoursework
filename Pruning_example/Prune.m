...
%   PRUNE will be removed in a future release. Use the PRUNE method of an object
%   returned by FITCTREE or FITRTREE instead.
%
...
%      view(t2);
%
%   See also CLASSREGTREE, CLASSREGTREE/TEST, CLASSREGTREE/VIEW, fitctree,
%   fitrtree.

%   Copyright 2006-2014 The MathWorks, Inc.

warning(message('stats:obsolete:ReplaceThisWithMethodOfObjectReturnedByEitherOr','classregtree','prune','fitctree','fitrtree'));
...
if nargin==2
    ...
    defaults = {[]      []      []      'error'};
    [level,nodes,alpha,crit] = ...
        internal.stats.parseArgs(okargs,defaults,varargin{:});
    pruneflags = ~isempty(level) + ~isempty(nodes) + ~isempty(alpha);
    if pruneflags>1
        error(message('stats:classregtree:prune:ParameterConflict'));
    end
end
...
if ~isempty(level)
   if numel(level)~=1 || ~isnumeric(level) || level<0 || level~=round(level)
      error(message('stats:classregtree:prune:BadLevel'))
   end
end
if ~isempty(nodes)
   nodes = nodes(:);
   if ~isnumeric(nodes) || any(nodes<1) || any(nodes~=round(nodes))
      error(message('stats:classregtree:prune:BadNodes'));
   end
end
if ~isempty(alpha)
    if ~isnumeric(alpha) || numel(alpha)~=1 || alpha<0
      error(message('stats:classregtree:prune:BadAlpha'));
    end
end

% Now do the pruning
if ~strncmpi(crit,Tree.prunecriterion,length(crit))
...
unsplitcost = risk(Tree,1:N,'criterion',crit,'mode','unsplit');
costsum = nodecost;
...
elseif strncmpi(crit,'impurity',length(crit))
